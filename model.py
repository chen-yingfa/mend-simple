...
class GTN(EditableModel):
    ...
    def load_state_dict(self, state_dict, strict: bool = True):
        ...
        if config != self.model.config:
            print("Loaded model config doesn't match current model config.")
    ...
    def edit(self, batch, condition=None, detach_history=False):
        ...
        if self.config.gtn.shared:
            ...
            transformed_factors = {
                n: self.gtn[str(tuple(self.get_shape(p)))](
...
if __name__ == "__main__":
    ...
    orig_param = [
        p
        for (n, p) in gtn.model.named_parameters()
    ...
    print(gtn(x, labels=x).loss, edited(x, labels=x).loss, edited2(x, labels=x).loss)
